...
function populateDropdowns() {
    ...
    if (sdrSelect) {
        ...
        [...DATA_CACHE.sdr, ...DATA_CACHE.bant].forEach(i => {
            if(i.sdr_name && (i.responsible_user_id || i.sdr_id)) unique.set(i.sdr_name, i.responsible_user_id || i.sdr_id);
        });
        unique.forEach((id, name) => {
            const opt = document.createElement('option'); opt.value = id; opt.textContent = name; sdrSelect.appendChild(opt);
        });
    }
...
function renderExecutiveView() {
    ...
    if (GlobalFilter.sdrId !== 'all') targetMetas = targetMetas.filter(m => String(m.id) === String(GlobalFilter.sdrId));
